...
def get_categories():
    categories = query_db("SELECT id, category_name FROM categories ORDER BY popularity DESC")
...
def get_products():
    ...
    total_count = query_db(params["count_query"], tuple(params["count_args"]), one=True)["count"]
...
def track_event():
    data = request.get_json()
    if data.get("event_type") not in {"view_product", "click_category", "search", "add_to_cart", "purchase"}:
        return jsonify({"error": "Invalid event type"}), 400

    data["timestamp"] = data.get("timestamp", datetime.datetime.now(datetime.UTC).isoformat())
